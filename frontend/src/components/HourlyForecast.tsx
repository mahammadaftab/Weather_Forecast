    ...
    <div className="mb-8">
      ...
      <div className="flex overflow-x-auto pb-4 space-x-4">
        {forecast.map((item, index) => (
          <div key={index} className="bg-white/20 backdrop-blur-sm rounded-2xl p-4 min-w-[120px] text-center text-white">
            <p className="mb-2 font-medium">{item.time}</p>
            <div className="text-3xl mb-2">{item.icon}</div>
            <p className="text-xl font-semibold mb-1">{Math.round(item.temperature)}°</p>
            <p className="text-white/80 text-sm mb-1">Feels: {Math.round(item.feelsLike)}°</p>
            {item.precipitation > 0 && (
              <p className="text-blue-300 text-sm mb-1">{item.precipitation}%</p>
            )}
            <div className="flex justify-center items-center text-white/70 text-sm mt-1">
